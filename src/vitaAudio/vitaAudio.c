...
static int32_t readWavHeader( AudioHandler* audioHandler )
	...
	if ( ret < 0 ) 	{ printf( "sceIoLseek() 0x%08x\n", ret ); goto term; }
	...
	while ( readSize > 0 )
		...
		if ( ret < 0 ) { printf ( "ERROR: sceIoRead () 0x%08x\n", ret ); goto term; }
	...
	if ( ret < 0 ) 	{ printf( "ERROR: _sceParseWaveHeader() 0x%08x\n", ret ); goto term; }
...
static int32_t soundThread( uint32_t args, void *argc )
	...
	if ( portId < 0 ) 	{ printf( "Error: sceAudioOutOpenPort() 0x%08x\n",portId ); goto term; }
	...
	for (;;)
		...
		if ( GetBufferCurrentCount( audioHandler->fileBuffer ) >= USER_GRAIN * STEREO * sizeof(short) )
			...
			if ( ret < 0 ) { printf ( "Error: sceAudioOutOutput() 0x%08x\n",ret ); break; }
			side ^= 0x01;
			} 
		else
			{
	...
	if ( ret < 0 ) { printf( "Error: sceAudioOutReleasePort() 0x%08x\n",ret ); goto term; }
		...
		if ( audioHandler->memory == 1 )
			...
			while ( readSize > 0 || audioHandler->endflag == AUDIO_STATUS_END )
				...
				if ( capacity > 0 )
					...
					if ( capacity < 0 ) { printf( "ERROR: sceIoRead () 0x%08x\n", capacity ); goto term; }
		...
		while ( count > 0 || audioHandler->endflag == AUDIO_STATUS_END )
			...
			if ( capacity > 0 )
				...
				if ( buffer_len > 0 )
					...
					if ( count < 0 ) { printf( "ERROR: sceIoRead () 0x%08x\n", ret ); goto term; }
					if ( error == VORBIS_ERROR_DECODE_RECOVERED ) { printf( "Warning: possible corruption at sample \n" ); goto term; }
					else if ( error && error != VORBIS_ERROR_STREAM_END )
						{
						// STREAMING ERROR
						if ( 		error == VORBIS_ERROR_DECODE_FAILED ) 				{ printf( "Decode failed at sample d\n" ); }
						else if (	error == VORBIS_ERROR_INSUFFICIENT_RESOURCES) 		{ printf( "Out of memory at sample d\n" ); }
						else 															{ printf( "Unexpected libnogg error %d at sample \n", error ); }
						goto term;
						}
...
int32_t InitializeAudio( AudioHandler* audioHandler )
	...
	// ERROR HANDLING
	if ( audioHandler->buffer  	  == NULL )  	{ printf( "ERROR: malloc buffer\n"  	); ret = AUDIO_ERROR_OUT_OF_MEMORY; goto error; }
	audioHandler->header 		= (uint8_t*)malloc(HEADER_SIZE);
	if ( audioHandler->header  	  == NULL ) 	{ printf( "ERROR: malloc header\n"  	); ret = AUDIO_ERROR_OUT_OF_MEMORY; goto error; }
	audioHandler->readBuf 		= (uint8_t*)malloc(READBUF_SIZE);
	if ( audioHandler->readBuf    == NULL ) 	{ printf( "ERROR: malloc readBuf\n"  	); ret = AUDIO_ERROR_OUT_OF_MEMORY; goto error; }
	audioHandler->fileBuffer 	= (FileBuffer*)malloc(sizeof(FileBuffer));
	if ( audioHandler->fileBuffer == NULL ) 	{ printf( "ERROR: malloc fileBuffer\n" 	); ret = AUDIO_ERROR_OUT_OF_MEMORY; goto error; }
	ret 						= CreateBuffer(audioHandler->fileBuffer, audioHandler->buffer, FILEBUF_SIZE);
	if ( ret < 0 )  							{ printf("ERROR: CreateBuffer () 0x%08x\n", ret); goto error; }
...
int32_t LoadOgg( AudioHandler* audioHandler, char* filename, SceUInt32 mode, SceUInt32 memory )
	...
	if ( !handle )
		{
		// ERROR HANDLING
		if ( error == VORBIS_ERROR_INSUFFICIENT_RESOURCES 	) 	{ printf( "LoadOgg Out of memory\n" ); }
		else if ( error == VORBIS_ERROR_STREAM_INVALID 		) 	{ printf( "LoadOgg Invalid stream format\n" ); }
		else if ( error == VORBIS_ERROR_STREAM_END 			) 	{ printf( "LoadOgg Unexpected EOF\n" ); }
		else if ( error == VORBIS_ERROR_DECODE_SETUP_FAILED ) 	{ printf( "LoadOgg Failed to initialize decoder\n" ); }
		else 													{ printf( "LoadOgg Unexpected libnogg error %d\n", error ); }
		ret 		= -1;
		goto error;
		}
	// READ HEADER
...
int32_t LoadWav( AudioHandler* audioHandler, char* filename, SceUInt32 mode, SceUInt32 memory )
	...
	if ( ret < 0 ) { printf("ERROR: readHeader() 0x%08x\n", ret); goto term; }
	if ( audioHandler->memory == 1 )
		{
		...
		if ( audioHandler->fillBuf == NULL ) { printf( "ERROR: malloc fillBuf\n" ); ret = AUDIO_ERROR_OUT_OF_MEMORY; goto error; }
...
int32_t PlayAudio( AudioHandler* audioHandler )
	...
	if ( ret < 0 ) { printf("Error: [readThread]sceKernelStartThread 0x%08x\n", ret); goto term; }
	ret 			= sceKernelStartThread( audioHandler->soundThread, sizeof(audioHandler), &audioHandler );
	if ( ret < 0 ) { printf( "Error: [soundThread]sceKernelStartThread 0x%08x\n", ret ); goto term; }
	goto term;
